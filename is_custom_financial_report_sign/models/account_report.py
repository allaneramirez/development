    ...
    def export_to_xlsx(self, options, response=None):
        ...
        try:
            ...
            if render_context.get('company_vat'): header_lines.append(
                f"VAT: {render_context.get('company_vat', '')}")  # OJO: Puedes cambiar "VAT" por "NIT"
            ...
            # --- 4. ELIMINAR FILAS DE ENCABEZADO ORIGINALES (SI EXISTEN) ---
            rows_to_delete = []
            max_rows_to_check = 5
            for row_idx in range(2, min(sheet.max_row + 1, 2 + max_rows_to_check)):
                ...
                if isinstance(cell_value, str):
                    ...
                    # Estas son las cadenas que Odoo suele poner en su cabecera
                    if "partir de" in cell_text or "balance" == cell_text or self.name.lower() in cell_text:
            ...
            # --- 6. AGREGAR CERTIFICACIÓN Y FIRMAS ---
            ...
            if signatures:
                ...
                for i, (name, title) in enumerate(signatures):
                    ...
                    sign_line_row = current_row;
                    sign_name_row = current_row + 1;
                    sign_title_row = current_row + 2
                    try:
                        ...
                        cell_line = sheet.cell(row=sign_line_row, column=col_start);
                        ...
                        cell_name = sheet.cell(row=sign_name_row, column=col_start);
                        ...
                        cell_title = sheet.cell(row=sign_title_row, column=col_start);
                        ...
                        _logger.error(
                            f"Error al procesar/combinar celdas para la firma {i + 1} (vertical): {merge_err}")
                        current_row = sign_title_row + 1
