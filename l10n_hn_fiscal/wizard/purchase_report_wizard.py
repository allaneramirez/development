...
class PurchaseReportWizard(models.TransientModel):
    ...
    def action_generate_pt_excel(self):
        """Genera el reporte PT en formato Excel"""
        self.ensure_one()
        ...
        detalle_isv = result.get('detalle_isv', [])

        f = io.BytesIO()
        libro = xlsxwriter.Workbook(f)
        hoja = libro.add_worksheet('PT Libro Compras')
        ...
        hoja.merge_range(0, 0, 0, num_columnas - 1, company.name or '', formato_encabezado_reporte)
        hoja.merge_range(1, 0, 1, num_columnas - 1, f'RTN: {company_rtn}', formato_encabezado_reporte)
        hoja.merge_range(2, 0, 2, num_columnas - 1, 'PT REPORTE DE COMPRAS', formato_encabezado_reporte)
        hoja.merge_range(3, 0, 3, num_columnas - 1, f'Período: Del {fecha_desde_str} al {fecha_hasta_str}', formato_encabezado_reporte)
        hoja.merge_range(4, 0, 4, num_columnas - 1, 'Expresado en Lempiras', formato_encabezado_reporte)

        base_columnas = [
            'Número', 'Tipo', 'Fecha', 'RTN', 'Nombre/Razón Social del Proveedor',
            'Descripción', 'CAI', 'Fecha Límite de Emisión', 'Número de Documento',
            'Establecimiento', 'Punto de Emisión', 'Tipo de Documento', 'Correlativo',
            'Total', 'Importe Exento', 'Importe Exonerado',
        ...
        for linea in lineas:
            ...
            hoja.write(y, 4, linea.get('proveedor', ''), formato_datos_texto)
            hoja.write(y, 5, linea.get('descripcion', ''), formato_datos_texto)
            hoja.write(y, 6, linea.get('cai', ''), formato_datos_texto)
            hoja.write(y, 7, linea.get('emition_limit'), formato_fecha)
            hoja.write(y, 8, linea.get('numero_correlativo', ''), formato_datos_texto)
            hoja.write(y, 9, linea.get('establecimiento', ''), formato_datos_texto)
            hoja.write(y, 10, linea.get('punto_emision', ''), formato_datos_texto)
            hoja.write(y, 11, linea.get('tipo_doc_ref', ''), formato_datos_texto)
            hoja.write(y, 12, linea.get('correlativo_ref', ''), formato_datos_texto)
            hoja.write(y, 13, linea.get('total', 0.0), formato_numero)
        ...
        hoja.set_column(6, 12, 20)
        hoja.set_column(13, 21, 18)
        hoja.set_column(22, 22, 18)
        hoja.set_column(23, 24, 35)
        hoja.set_column(25, 27, 25)

        libro.close()
        datos_binarios = base64.b64encode(f.getvalue())
        self.write({
            'archivo': datos_binarios,
            'name': 'pt_reporte_compras.xlsx'
        })
