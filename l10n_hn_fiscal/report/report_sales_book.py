...
class ReportSalesBook(models.AbstractModel):
    _name = 'report.l10n_hn_fiscal.report_sales_book'
    _description = 'Reporte de Libro de Ventas Honduras'
    ...
    def process_invoices(self, datos):
        """
        Procesa las facturas y calcula los totales según los impuestos configurados
        """
        totales = {
            'num_facturas': 0,
            'total_exento': 0,
        ...
        # Construir el dominio de búsqueda (incluir facturas y notas de crédito en estado posted y cancel)
        domain = [
        ...
        for invoice in invoices:
            ...
            for line in invoice.invoice_line_ids:
                ...
                if line.account_id:
                    ...
                    account_lines.append(f"[{account_code}] {account_name}")
                
                # Calcular impuestos usando compute_all como en Guatemala
                ...
                signo = -1 if invoice.move_type == 'out_refund' else 1
            ...
            linea = {
                ...
                'detalle_haber': '\n'.join(credit_lines),
                'producto_detalle': '\n'.join(account_lines),
                'condicion_pago': invoice.invoice_payment_term_id.name or '',
                'total': invoice_total,
            }
        ...
        lineas = sorted(lineas, key=lambda x: (x['tipo_documento'], x['fecha'] or ''))
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # Color de los totales: 50% de transparencia (alpha 0.5)
        ...
        return {
            'doc_ids': docids or [],
            'doc_model': 'sales_report_wizard',
            'data': form_data,
            'docs': self.env['sales_report_wizard'].browse(docids) if docids else self.env['sales_report_wizard'],
            'result': result,
